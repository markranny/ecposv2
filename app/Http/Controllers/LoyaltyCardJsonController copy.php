...
class LoyaltyCardJsonController extends Controller
{
    ...
    public function updatePoints(Request $request, LoyaltyCard $loyaltyCard, $points)
    {
        // Only HEADOFFICE users can modify points
        if (Auth::user()->storeid !== 'HEADOFFICE') {
            return response()->json([
                'message' => 'Only HEADOFFICE users can modify points'
            ], 403);
        }

        // Validate points from URL parameter
        ...
        try {
            DB::transaction(function () use ($loyaltyCard, $points) {
                ...
                $loyaltyCard->transactions()->create([
                    'type' => $newPoints > $oldPoints ? 'earn' : 'redeem',
                    'points' => abs($newPoints - $oldPoints),
                    'description' => 'Points manually updated',
