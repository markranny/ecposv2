    ...
    /* public function getMetrics(Request $request)
{
    try {
        ...
        \Log::info('Metrics Request Received', [
            ...
            'total_stores_selected' => count($validated['stores'] ?? [])
        ]);

        $startDate = $validated['start_date'] ?? now()->subYears(10)->toDateString();
        ...
        if (!empty($validated['stores'])) {
            ...
            \Log::info('Metrics Query Filtered by Stores', [
                'selected_stores' => $validated['stores']
            ]);
        }
        ...
        \Log::info('Calculated Metrics', [
            ...
            'total_netsales' => $metrics['totalNetsales'],
            'date_range' => [
                'start' => $startDate,
        ...
        \Log::error('Metrics Validation Error', [
            'errors' => $e->errors(),
            'input' => $request->all()
        ]);
