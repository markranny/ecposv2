...
public function inventory(Request $request)
    {
        ...
        $query = StockCountingTrans::select(
            'inventtables.itemid',
            'inventtables.itemname',
            // Beginning Balance for the previous day
            DB::raw('(SELECT SUM(counted) FROM stockcountingtrans s2 
                     WHERE s2.itemid = inventtables.itemid 
                     AND DATE(s2.transdate) = "'.$yesterday.'") as beginning'),
            ...
            // Item count for today
            DB::raw('(SELECT SUM(counted) FROM stockcountingtrans s3 
                     WHERE s3.itemid = inventtables.itemid 
                     AND DATE(s3.transdate) = "'.$today.'") as item_count'),
            
            // Ending balance calculation
            DB::raw('(
                COALESCE((SELECT SUM(counted) FROM stockcountingtrans s2 
                         WHERE s2.itemid = inventtables.itemid 
                         AND DATE(s2.transdate) = "'.$yesterday.'"), 0) + 
            ...
            // Variance: Item Count - Ending Balance
            DB::raw('(
                (SELECT SUM(counted) FROM stockcountingtrans s3 
                 WHERE s3.itemid = inventtables.itemid 
                 AND DATE(s3.transdate) = "'.$today.'") - 
                (
                    COALESCE((SELECT SUM(counted) FROM stockcountingtrans s2 
                              WHERE s2.itemid = inventtables.itemid 
                              AND DATE(s2.transdate) = "'.$yesterday.'"), 0) + 
        ...
        // Filtering for variance != 0
        ->havingRaw('variance != 0');
